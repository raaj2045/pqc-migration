// Ethereum-native leg, step 3 (EVM side): prove the Cosmos acknowledgement
// of step-native-recv.js back to ICS26Router, closing out the packet.
//
// Usage: node step-native-ack.js <cosmos-recv-txhash>
//
// Structurally identical to step-redeem-ack.js: the acknowledgement
// membership proof and the client update behind it are real SP1 Groth16
// proofs produced by proof-api and verified on chain by SP1VerifierGroth16 —
// same path, same proof-api, just repointed at native-send.json so this
// flow's handoff files never collide with the redemption cycle's.
const fs = require("fs");
...
if (!RECV_TX) {
  console.error("usage: node step-native-ack.js <cosmos-recv-txhash>");
  process.exit(2);
}
...
(async () => {
  ...
  const send = JSON.parse(fs.readFileSync(env.file("native-send.json")));
  ...
  // The native-send packet travels EVM -> Cosmos, but this relay travels
  // Cosmos -> EVM (it carries the acknowledgement home). proof-api's client
  // ids describe the relay direction, so they are the reverse of the
  // packet's: source is the Cosmos-side client, destination the EVM-side one.
  ...
  // The EVM-side commitment for the outbound native-send packet is cleared
  // once the acknowledgement is accepted; that is what closes the packet out.
  ...
  fs.writeFileSync(env.file("native-ack.json"), JSON.stringify({
  ...
  console.log(`wrote ${env.file("native-ack.json")}`);
})();
