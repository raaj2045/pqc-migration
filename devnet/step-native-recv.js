// Ethereum-native leg, step 2 (Cosmos side): prove the EVM packet commitment
// from step-native-send.js with the real Ethereum light client
// (cw-ics08-wasm-eth) and submit MsgRecvPacket. transfer/keeper OnRecvPacket
// finds no existing denom trace for this (unmapped) base denom, auto-registers
// it (SetDenom/SetDenomMetadata) and mints a new ibc/<hash> voucher to the
// packet's receiver — no pre-registration needed on the Cosmos side.
//
// This is structurally identical to step-redeem-recv.js: that script already
// proves an EVM-originated packet to Cosmos via the same client. The only
// difference is which JSON handoff file it reads/writes, so the two flows
// don't collide.
//
...
(async () => {
  ...
  fs.writeFileSync(env.file("msg-native-recv.json"), JSON.stringify(msg, null, 2));

  const [txCmd, ...txArgs] = env.SENDTX_CMD.split(/\s+/);
  const out = execFileSync(txCmd,
    [...txArgs, env.file("msg-native-recv.json"), SIGNER_KEY || env.RELAYER_KEY, "3000000"],
    { encoding: "utf8" });
  console.log(out.trim());
  if (!REPLAY) fs.writeFileSync(env.file("native-recv.json"), JSON.stringify({ useSlot }, null, 2));
})();
