...
CONSTANTS
    Seqs,        \* packet sequence numbers, e.g. 1..2
    Heights,     \* consensus-state heights, e.g. 1..2
    Amount,      \* every transfer moves this many units
    InitialA     \* chain A's user starts with this many units

VARIABLES
    \* ---- chain A (source) ----
    balA,        \* A's user balance
    escrow,      \* A's escrow account
    commitments, \* seq -> TRUE when a packet commitment is stored on A
    committedAt, \* height -> set of seqs whose commitment is in A's state at that height
    ackedA,      \* seqs whose acknowledgement A has processed (commitment deleted)

    \* ---- chain B (destination) ----
    vouchers,    \* B's user voucher balance
    receipts,    \* seqs for which B has stored a packet receipt
    trusted,     \* heights B's light client currently trusts
    credited,    \* seqs for which B actually credited the receiver
...
TypeOK ==
    ...
    /\ height \in 1..Cardinality(Heights)
